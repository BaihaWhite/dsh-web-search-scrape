    ...
    const FIELDS = [
      {
        ...
        kind: 'select',
        label: '搜索后端',
        hint: 'local = 本地六档抓取（无需 API key）；official = 内置 DeepSeek 搜索（走 API，端点/模型在「Web search」卡片配置）。切换即时生效，无需重启。',
        options: [
          { value: 'local', label: '本地六档抓取（web-scrape）' },
          { value: 'official', label: '官方 DeepSeek 搜索' },
        ],
      },
      {
        field: 'deepTool',
